...
simulate m = foldr moveDown m beams
  where beams = map fst . filter ((==TachyonBeam) . snd) . Map.toList $ m
        moveDown (x,y)
          | isNothing maybeBelow = id
          | below == Empty = Map.insert (x,y+1) TachyonBeam
          | below == Splitter = useSplitter
          | below == UsedSplitter = useSplitter
          | below == TachyonBeam = id
          | otherwise = error ("Unexpected tile " ++ show below)
          where maybeBelow = Map.lookup (x,y+1) m
                below = fromJust maybeBelow
                useSplitter = Map.insert (x, y+1) UsedSplitter . Map.insert (x-1, y+1) TachyonBeam . Map.insert (x+1, y+1) TachyonBeam
